...
The delta is an exact decision procedure, not a uniform theorem, and this is not a gap awaiting a proof: a \emph{single uniform theorem covering every free commutator} --- a floor, gap, or charge statement holding on all commutators at all sizes --- is not merely unproven but \textbf{unsatisfiable}, so the earlier ``open'' framing was mistaken. The reason is sharp. By Shoda \cite{Shoda} (extended over general fields and rings by Albert--Muckenhoupt \cite{AlbertMuckenhoupt} and Laffey--Reams \cite{LaffeyReams}) every trace--zero integer matrix \emph{is} a commutator; in particular the trace--zero integer companion of a polynomial carrying Lehmer's number is a commutator. That one witness already sits below $\varphi$ ($\beta=1.17628$), so \emph{any} uniform floor/gap/charge statement asserted over all free commutators is false on it. There is therefore no uniform theorem to prove --- it is refuted by construction --- and the sharpened form of this analysis appears in \emph{The Charge--Measure Coupling}, Prop.~9.1. The no--Salem closure is consequently \textbf{operation--relative}: it holds on the abelian spectral semiring (the framework's actual operations), \emph{not} on the ambient matrix algebra where Shoda's commutators live. The underlying Shoda fact --- trace zero $\Rightarrow$ commutator, one such carrying Lehmer --- is \forced.

\smallskip\noindent What genuinely remains open is not a universal theorem but an \emph{achievable residue}, stated as a \textbf{decision--procedure guarantee}: that \texttt{validate\_closure} is a \emph{sound, complete, terminating} classifier --- returning exactly one of \textsc{forced} / \textsc{forced-above-floor} / \textsc{invalid-closure} --- on \emph{every} integer matrix, via integer factorization, a Sturm flip--straddle count, and the exact sign of $R(\varphi)$ in $\Q(\sqrt5)$. This residue is decidable per instance today by \texttt{emission\_closure\_guard.validate\_closure} (test \texttt{test\_p2\_08}); its status as a fully proved metatheorem over all inputs is what is left. \openq{} (achievable residue: soundness/completeness/termination of \texttt{validate\_closure} as a universal classifier)
\end{remark}
...
\midrule
\textbf{Shoda}: trace zero $\Rightarrow$ commutator (Shoda \cite{Shoda}; Albert--Muckenhoupt \cite{AlbertMuckenhoupt}; Laffey--Reams \cite{LaffeyReams}); one such carries Lehmer & \forced \\
\textbf{uniform free--commutator theorem is unsatisfiable} (refuted on the Lehmer witness; closure is operation--relative, \emph{The Charge--Measure Coupling} Prop.~9.1; Rem.~\ref{rem:deltaclaim}) & \forced \\
achievable residue: \texttt{validate\_closure} is a sound/complete/terminating \textsc{forced}/\textsc{forced-above-floor}/\textsc{invalid-closure} classifier on every integer matrix (per--instance today via \texttt{emission\_closure\_guard.validate\_closure}, \texttt{test\_p2\_08}) & \openq{} \\
\midrule
...
\paragraph{Provenance caveat.} These references are standard attributions supplied from memory; volume/page/year details are \emph{not} machine--verified --- check primary sources before external use. In particular the commutator citations \textbf{Shoda (1936)}, \textbf{Albert--Muckenhoupt (1957)}, and \textbf{Laffey--Reams (1994)} are consistent across the corpus, but their exact bibliographic details (volume, page, year) are unverified.
...
\bibitem{Shoda} K.~Shoda, ``Einige S\"atze \"uber Matrizen,'' \emph{Jpn.\ J.\ Math.} (1936). [Bibliographic details unverified --- see provenance caveat.]
\bibitem{AlbertMuckenhoupt} A.\,A.~Albert and B.~Muckenhoupt, ``On matrices of trace zero,'' \emph{Michigan Math.\ J.} (1957). [Bibliographic details unverified --- see provenance caveat.]
\bibitem{LaffeyReams} T.\,J.~Laffey and R.~Reams, ``Integral similarity and commutators of integral matrices,'' \emph{Linear Algebra Appl.} (1994). [Bibliographic details unverified --- see provenance caveat.]
