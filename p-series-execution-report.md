...
| P3 | every certified Pisot's Rat scan = {Φ₁^deg} | fresh scans deg 2–4 (7+37+103) + archived deg 5–7 + new box | **2662/2662** (1117 + 1545, per-run scan executions; the nested quintic boxes are not deduplicated); [FORCED] by Thm 7.13 and observed without exception |
| P4 | totally-real Pisot ⇒ θ ≥ φ, equality only at degree 2 | fresh sweep deg 2–3 ([−3,3]ⁿ), deg 4 re-census, deg 5–7 + [−4,4]⁵ populations | **13/13 certified exactly** (sign of P(φ) in Z[φ]): θ > φ twelve times, θ = φ exactly once, at x²−x−1; zero falsifiers; stays [OPEN] as a general statement |
...
**P4 (first standalone execution).** Populations: degree 2, [−3,3]²: 7
Pisots, all totally real (including x²−3x+1 = minpoly(φ²) — see erratum E1 below); degree 3, [−3,3]³: 37 Pisots, 5 totally real;
...
Result: twelve strict certificates (including P(φ) = 1−√5 < 0 for θ = φ²),
one equality (the golden minimal polynomial), zero falsifiers.
...
**P3 aggregate.** Fresh complete Rat scans for degrees 2–4 (147 instances)
plus every archived record: 2662/2662 = {Φ₁^deg}, no Φ₂, no higher content
...
affirmatively). Forced-and-observed: P3 (2662/2662), P5. Open with
strictly enlarged zero-falsifier evidence: P1 (2016 live instances),
P4 (13 totally-real instances, equality pattern exactly as predicted),
...
**E1.** The first run's degree-2 census read 6; the correct count is **7**.
`x²−3x+1 = minpoly(φ²)` is reciprocal *and* Pisot, and the "reciprocal ⇒
not Pisot" disposal is `[FORCED]` only for degree ≥ 3; the instance sits in
the empty-chain population and was silently dropped. Fixed in
`p-series/p_census.py` (degree guard + exact-discriminant decision for
degree-2 self-inversive candidates), `p4_pisot2_box3.jsonl` re-pinned
(7 rows), all E1-cascaded numbers above updated (P4 13, P3 2662). Degree
≥ 3 outputs verified unchanged under the patched library. See
`p-series/ERRATA.md`.
