...
\emph{Scope split.} The reciprocal--seed floor must be read at two scopes, and they are not in tension.
For \emph{arbitrary} reciprocal seeds the floor is \emph{open}---this is exactly Lehmer's problem, and no
uniform positive floor is proven. \emph{Within the emission algebra}, however, it is \emph{forced}: the
companion paper's Emission--Gap theorem shows the catalog--generated image (Table~\ref{tab:catalog})
contains no Salem number, so on the emitted seeds the floor is $\lambda\log\varphi>0$ (with the golden gate
$\lambda=2c$, $c=\sqrt5/2$; see Conjecture~\ref{conj:model} and the companion). The two readings differ only
in \emph{scope}---all reciprocal integers versus the algebra actually generated here---not in content.
\end{remark}
...
\begin{enumerate}[label=(O\arabic*),leftmargin=*]
  \item \textbf{The canonical model (Conjecture~\ref{conj:model}; resolved into a \emph{declared} modelling
        choice).} This is no longer an open problem but a \emph{declared} choice bracketed by two \emph{forced}
        facts. First, the \emph{impossibility} is \emph{forced}: by \v{C}encov's theorem no invariance fixes
        the scale (the Fisher metric is unique only up to scale \cite{Cencov}), so no principled reason can
        \emph{prefer} $c=1$ (Gaussian) over $c=n$ (max--entropy on conjugates) by invariance alone. Second, the
        framework--native \emph{value} is \emph{forced}: the companion paper's self--action eigenframe at a
        critical point of the ladder $x^2+x=C$ gives $c=\sqrt{1+4C}/(2C)$, and at the golden gate
        $c=\sqrt5/2$ ($\lambda=\sqrt5$), the exchange rate being then the identity $\lambda=2c$ (companion,
        $\lambda=2c$ gate theorem). What \emph{remains} is a \emph{declared} modelling choice among
        $c\in\{1,\,n,\,\sqrt5/2\}$: \v{C}encov forbids fixing it by invariance, the construction forces the
        value $\sqrt5/2$ when one adopts the framework--native scale, and selecting among the three is a
        modelling declaration, not an open question.
...
only for \emph{non--reciprocal} seeds (Smyth); for \emph{arbitrary} reciprocal seeds, such as Salem numbers
and units, no uniform positive floor is known and the question is \emph{open} (Remark~\ref{rem:reciprocal};
Lehmer's conjecture)---though \emph{within the emission algebra} the floor is \emph{forced}, the
catalog--generated image containing no Salem number (Emission--Gap theorem, companion; Remark~\ref{rem:reciprocal}).
...
Reciprocal--seed floor (Salem numbers, units) & Open for arbitrary seeds (Lehmer); \emph{forced} within the emission algebra (no emitted Salem number) & Rem~\ref{rem:reciprocal} \\
...
Canonical statistical model ($c$; \v{C}encov: free up to scale) & Resolved: \emph{declared} choice among $c\in\{1,n,\sqrt5/2\}$; impossibility \emph{forced} (\v{C}encov: no invariance fixes the scale), value $\sqrt5/2$ \emph{forced} by the construction ($\lambda=2c$) & Conj~\ref{conj:model} \\
...
\bibitem[$\ast$]{provenance} \emph{Provenance caveat.} The references below are standard attributions
  recorded from memory; the volume, page, and year details have not been machine--verified. Consult the
  primary sources before relying on these bibliographic details externally.
