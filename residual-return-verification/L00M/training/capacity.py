...
def capacity_decision(min_poly, residual_norm, budget: Budget, *, effective_degree=None, floor=0,
    ...
    With `info_threshold=True` the gain/cost criterion of A3.P2b is applied:
        ...
        floor  = (ambient_degree if degree_aware else 1) * lam * log(mu)  =  2c * log(mu)   -- mult IS the
                 conformal constant c (Cencov: declared, unique only up to scale; c=1 Gaussian / c=n
                 degree-invariant), and lam=2 is the structural "2" of the DERIVED identity lambda = 2c
                 (residual_return_learning.tex Thm 4.6). Smyth-/Lehmer-/Dobrowolski-derived for mu.
    ...
    says so in the reason. `lam`/`mu`/`dobrowolski_c` are named, documented, tunable parameters; `lam = 2c` is the DERIVED exchange rate (companion Thm 4.6),
    with `c` the declared conformal scale (Cencov: free up to scale) carried here by
    `mult = (ambient_degree if degree_aware else 1)`, c in {1, n}."""
    if isinstance(residual_norm, float) or isinstance(floor, float):
        raise TypeError("residual_norm and floor must be exact (int/Fraction), not float -- the gate is exact (G8)")
    ...
    with _mp.workdps(50):
        ...
        if not recip:
            ...
            floor_iv = iv.mpf(int(mult)) * iv.mpf(int(lam)) * log_mu
            ...
            if d >= 2:
                ...
                if m_lb.a > 1:                           # only a CERTIFIED-positive lower bound counts
                    floor_iv = iv.mpf(int(mult)) * iv.mpf(int(lam)) * iv.log(m_lb)
        cost_upper_iv = iv.mpf(int(mult)) * iv.mpf(int(lam)) * (iv.log(iv.mpf(int(landau_bound_sq(min_poly)))) / 2)  # mult=c: gate is consistently lambda=2c
    ...
    tag = (f"[info_threshold lam={lam} degree_aware={degree_aware} n={mult} floor={floor_kind}; "
