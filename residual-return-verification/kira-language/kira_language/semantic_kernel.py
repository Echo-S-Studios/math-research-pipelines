...
REWIRED for kira-language (2026-06-23): this is the project's BASE copy of the external
candidate `candidates/KL_DTA__Vsemantic_kernel.py` (kept pristine). Only honesty-critical
fixes are applied here -- decision (b): the interpretive prose on observer/framework is split
into INTERPRETIVE rows (the proven equations stay THEOREM), and orbit_regimes states its
iteration convention so the exponent is unambiguous. Still FLOAT (the exact-Fraction core is
the next increment). One-way disjoint: this module does NOT import loom; L00M never imports it.
"""
...
# (b) FIREWALL: only these jurisdictions may cross the wire as fact (decision (b), Ace-ratified).
# INTERPRETIVE / FALSE_AS_STATED are recorded (tagged) but NEVER wired as fact.
WIRED_JURISDICTIONS = (THEOREM, COMPUTED)
...
LAW_BANK: Tuple[Stmt, ...] = (
    ...
    # (b) firewall fix: the proven EQUATION stays THEOREM with a FACTUAL invariant/residual;
    # the interpretive prose is split into the INTERPRETIVE rows below (never on a THEOREM row).
    Stmt("observer", "Obs_K(X)=M(X)-τ(M(X))·1", "Obs_K ≡ R_K (identically the residual operator)", "τ(Obs_K(X))=0", THEOREM),
    Stmt("framework", "FW_K(X)=Resid_K(X)", "FW_K ≡ Obs_K ≡ R_K (definitional aliases)", "alias residual = 0", THEOREM),
    Stmt("observer_reading", "the observer IS the residual, not an external spectator", "interpretive framing of Obs_K", "no wired claim", INTERPRETIVE),
    Stmt("framework_reading", "the framework IS the residual ledger, not a wrapper around residual", "interpretive framing of FW_K", "no wired claim", INTERPRETIVE),
    ...
    Stmt("orbit_regimes", "λ(Mⁿ(X))=λ(M(X))^(2^(n-1)) for n≥1 [convention: M⁰=identity, M¹=first fold]", "collapse/gate/diverge", "eigenvalue threshold", THEOREM),
