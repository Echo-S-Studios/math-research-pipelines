...
itself, and which part only to a chosen reference ray? We answer by
refactoring the emission algebra's charge into a purely relational invariant
and determining exactly how much survives.
The companion note on the charge--measure coupling attaches to each object of the
emission algebra --- a finite, conjugation-closed multiset of nonzero algebraic
integers --- an \emph{absolute} phase character: a charge group $\Z/n\Z$ and a charge
per conjugate, both read against the fixed reference ray $\Arg=0$. Here we refactor
the character so that charge attaches not to the object but to the \emph{relation
between oscillators}: the pair invariant $t_{\mathrm{rel}}(\alpha,\beta)=
\frac{1}{2\pi}\bigl(\Arg\alpha-\Arg\beta\bigr)\in\tors$, defined exactly when the
difference is rational. The relational data forms a $\tors$-valued groupoid cocycle
of which the absolute charge is a trivialization: the reference ray is a
gauge choice. The principal structural result is a rigidity theorem: because
integer polynomials have conjugation-closed spectra, an object all of whose
pairwise phase differences are rational already has all phases rational --- the
relational theory admits no new objects, and the absolute charge group is pinned to
one of two anchors $n\in\{m,2m\}$ over the relational group $\Z/m\Z$. The
composition laws ($\lcm$, CRT/Adams, safe $\Leftrightarrow$ coprime) descend
verbatim to the relational level. Where the
relational theory is genuinely larger is the sector the absolute theory discards:
charge-inadmissible objects carry a finite \emph{coherence type} (a partition of
the conjugates into rational-difference classes with cyclic relative-charge
groups), and we give a float-free decision procedure for it --- complete at the
level of equal-modulus shells, hence a complete decision of the full type for
Salem numbers --- via ratio resultants and cyclotomic contact. Executing it on
four exactly certified Salem numbers of degrees $4$, $6$, $8$, $10$ (including
$\beta_4$ and Lehmer's number), then exhaustively on the degree-$12$ family of
all $729$ monic reciprocal polynomials with coefficients in $\{-1,0,1\}$
($37$ certified Salem numbers up to sign twist): every certified instance is
\emph{relationally inert} --- no nontrivial rational phase relation exists
among its conjugates off the real axis --- and no two of the canonical four
are circle-locked (circle-locked: a rational
phase difference links unit-circle conjugates \emph{across} the two objects);
each verdict is a complete per-instance decision, not a general theorem, and
whether \emph{any} Salem number carries nontrivial coherence remains open
(problem~P1). Last --- and conditionally on the companion note's emission
gap, the one hypothesis this note does not prove beyond degree two --- the
parity-graded Mahler floor is evaluated and becomes reference-free: the
parity bit is the order-$2$ element
$t_{\mathrm{rel}}(\varphi,\varphi')=\tfrac12$ carried internally by the
golden pair, not the ray $\pi$, and a sign-twist involution absorbs the new
anchor sector, so the relational floor equals the absolute one; that
identity, and the attainments, are unconditional --- only the floor's
numerical values rest on the gap. All
computational claims are verified in exact integer arithmetic and cross-checked
in a second computer-algebra stack; throughout, unconditional proofs, results
conditional on the emission gap, and per-instance computations are tagged
apart.
\end{abstract}
...
\item \textbf{Coherence types of inadmissible objects} (\S\ref{sec:inert}). On the
  ...
  equal-modulus shells by a ratio-resultant / cyclotomic-contact procedure with a
  provable completeness bound. Executing it on four Salem numbers of degrees
  $4,6,8,10$ (including $\beta_4=x^4-x^3-x^2-x+1$ and Lehmer's polynomial),
  each certified Salem exactly by Sturm counts on its trace polynomial: all
  four are relationally inert --- the only rational phase relations are the
  tautological diagonal and the positivity-forced real pair $(\tau,1/\tau)$;
  an exhaustive degree-$12$ small-coefficient census adds $37$ further
  certified, uniformly inert instances --- a base, not a class (open
  problem~P1). \F{} per instance
\item \textbf{Cross-object locking} (\S\ref{sec:locking}). Two objects can be
  ...
  shells by a mixed ratio resultant. All six pairs among the four certified
  instances are \emph{not} circle-locked. \F{} per instance; the existence
  question for the Salem spectrum at large is the principal open problem. \Op
\end{itemize}
...
Appendix~\ref{app:quadEG} requires~\cite{CMC}, which is distributed with
...
procedure for it. The census infrastructure connects to the small-Salem
...
evidenced, not proved in full generality), \Op{} (open). Statements inheriting a
...
section; no connection to physical gauge theory is implied. The next section
shows that on the
...
is squarefree --- here each is irreducible --- and Lemma~\ref{lem:diag} uses
this.
...
family splits into $378=(729+27)/2$ twist-classes, of which $27$ are
twist-fixed --- an internal count check. Exactly $37$ twist-classes are
...
structure whatsoever off the real axis'' for these witnesses. Whether
\emph{every} Salem number is relationally inert is open (\S\ref{sec:open});
each instance is decidable by
Lemmas~\ref{lem:contact}--\ref{lem:complete}.
\end{corollary}
...
of ledger entries A--I, together with the round-three rows P and Q, were
recomputed independently in PARI/GP~2.15.4 via
\texttt{polresultant}, integer factorization, and \texttt{poliscyclo}; all
nine signatures agree with the \texttt{sympy} computations (ledger
...
\texttt{census\_deg12.py}, \texttt{cross\_check.gp},
\texttt{SHA256SUMS}); a DOI-archived deposit of
these artifacts alongside the manuscript is the intended distribution
channel, enabling verification by a distinct operator. Concretely:
\texttt{python3 relational\_charge\_probe.py} reruns entries A--H under
assertions (expected: the signatures of Appendix~A, e.g.\
$\{\Phi_1^{\,10}\}$ for Lehmer, about one second);
\texttt{python3 supplement\_round3.py} reruns O--S;
\texttt{python3 census\_deg12.py} reruns the census of
Theorem~\ref{thm:census} (about two minutes); and
\texttt{gp -q cross\_check.gp} reproduces the sixteen cross-engine
signatures. Any deviation from Appendix~A is a finding.
...
Not locked (Thm.~\ref{thm:notlocked}, Rem.~\ref{rem:allpairs}) & no pair among the four circle-locks & \F{} per instance\\
Census (Thm.~\ref{thm:census}) & $37/37$ degree-$12$ family Salems inert & \F{} per inst.; \C{} family\\
...
\begin{enumerate}
\item[(P1)] \textbf{Internal Salem coherence.} Does any Salem number have two
circle conjugates at rational phase difference? Each instance is decidable by
the contact scan; a survey over the small-Salem census
(cf.~\cite{Boyd1977, Mossinghoff}) is the natural next
computation. A negative answer in general would say the Salem spectrum is
relationally inert as a class --- the strongest possible form of its
charge-inadmissibility. Forty-one certified instances --- the four canonical
test cases of degrees $4$--$10$ (the minimal degree-$4$ Salem number and
Lehmer's number among them; Theorems~\ref{thm:inert}, \ref{thm:inertmore})
plus the $37$ of the exhaustive degree-$12$ family
(Theorem~\ref{thm:census}) --- are a base, not a class; the full small-Salem
census is the required step. No naturally occurring \emph{irreducible}
non-inert example is known --- Example~\ref{ex:tensorsq}'s witness is
composite by construction --- and finding one, or proving none exists, is
precisely this problem. Where to look: a torsion ratio $\zeta_M$ between
conjugates forces $\Q(\zeta_M)$ into the splitting field, so irreducible
candidates are Salem polynomials whose splitting fields contain roots of
unity of nontrivial order. By contrast, \emph{reducible} inadmissible
objects with nontrivial coherence provably exist: any tensor square
$O\otimes O$ of a Salem object contains the equal-angle non-real pair
$(\tau\lambda,\ \tau^{-1}\lambda)$, coherent with relative charge $0$
(Proposition~\ref{prop:tensor}(iii)), so it is not inert --- computed
concretely in Example~\ref{ex:tensorsq}. \Op
\item[(P2)] \textbf{Cross-Salem locking.} Do any two distinct Salem numbers
circle-lock? Equivalently: do the circle angles of the Salem spectrum, viewed in
$(\R/\Z)/\Q$, ever collide? Theorem~\ref{thm:notlocked} and
Remark~\ref{rem:allpairs} settle all six pairs among the certified instances
negatively. \Op
...
M & cross-engine replication & contact signatures of entries A--I, P, and Q
recomputed in PARI/GP~2.15.4 (\texttt{polresultant} $+$
\texttt{poliscyclo}); all sixteen identical to the \texttt{sympy}
signatures.\\[2pt]
...
$\{\Phi_1^{\,12}\}$. Runtime $93$\,s; single-engine
(\texttt{census\_deg12.py}).\\
\hline
\end{tabular}
\end{center}
...
one and intended for the same repository. So that the present note can be
...
math-research-pipelines technical note, 30 June 2026. Distributed with the
present note; intended for the project's papers drop-zone at
\texttt{https://echo-s-studios.github.io/math-research-pipelines/}.
Appendix~\ref{app:inherited} lists every statement imported from it. (DOI
deposit pending.)
...
companion note of this project. The bibliographic metadata of
...
\url{10.1090/s0025-5718-98-01006-0}.
