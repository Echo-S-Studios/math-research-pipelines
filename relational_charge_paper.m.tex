...
$t_{\mathrm{rel}}(\alpha,\beta)=t(\alpha)-t(\beta)\in\Q/\Z$, with
$t(\alpha)=\frac{1}{2\pi}\Arg\alpha$, defined whenever the difference is
rational --- and determine exactly how much
...
\item \textbf{The gauge structure} (\S\ref{sec:relational}). The relational charge
  ...
  \S\ref{sec:rigidity}: rigidity says the gauge is not free on the
  algebraic category --- an object's phases are pinned to rational values
  outright, the only structural slack being the anchor dichotomy
  $n\in\{m,2m\}$, itself absorbed object-by-object by the sign twist
  (Lemma~\ref{lem:twist}). \F
...
Notation follows~\cite{CMC} throughout. Because~\cite{CMC} is a technical note of the same project ---
DOI-archived but not peer-reviewed --- this note is organized so that its
verification burden is explicit, and the conditional content it supports is
confined to exactly two numerical values (Remark~\ref{rem:robust}). The unconditional
...
(Smyth~\cite{Smyth1986} and the literature following it, notably the
torsion-quotient program of Dubickas~\cite{Dub2012, Dub2017, Dub2021};
Remark~\ref{rem:degen} records the attribution); the present note
...
section; no connection to physical gauge theory is implied. A reader
uncomfortable with the vocabulary may substitute ``differences of
normalized arguments'' throughout without loss. The cocycle
...
the degree computation in Definition~\ref{def:rat} below does not (it needs
only monic and $p(0)\neq0$).
...
and are likewise inert (ledger entry~V). Each of the $341$ non-Salem
twist-classes carries an exact rejection certificate: $39$ vanish at
$x=\pm1$, $257$ fail the trace-polynomial Sturm pattern $(1,0,5)$, and $45$
are reducible (none fail only trace-polynomial irreducibility); together
with the $37$ Salem classes, $39+257+45+37=378$ accounts for every
twist-class (ledger entry~W).
\end{remark}
...
\begin{example}[A Pisot instance, and the $\nu$-criterion executed]
\label{ex:pisotnu}
(i)~The smallest Pisot number, the plastic number $\theta_0$ (the real root
of $x^3-x-1$), is relationally inert: its shells are $\{\theta_0\}$ and the
conjugate pair at modulus $\theta_0^{-1/2}$ (the root product is $-p(0)=1$),
pinning applies, and the complete contact scan returns $\{\Phi_1^{\,3}\}$
(ledger entry~W); for a cubic there is no cross-shell residue, so
Corollary~\ref{cor:pisot} closes the full type. (ii)~The $\nu$-criterion of
Remark~\ref{rem:scope}, executed symbolically on the group-drop quartic
$x^4+5x^2+5$: the cross-shell pair $(ia,\,ib)$ has $\mu=a/b\in\R_{>0}$, so
$\nu=\mu/\overline{\mu}=1$ exactly --- torsion of order $1$, hence coherent
with $t_{\mathrm{rel}}=0$, merging the two shells into the single class of
Example~\ref{ex:firsttype}. (iii)~On $\beta_4$'s cross-shell pair
$(\tau,\lambda)$: $\nu=\overline{\lambda}/\lambda=\lambda^{-2}$ is itself a
root of $\Rat_{\beta_4}$, so its torsion status is decided by the very scan
of ledger entry~G --- no contact, so the pair is incoherent, as pinning
independently forces. \F
\end{example}

\begin{example}[The $\nu$-criterion with a nontrivial verdict: $x^4-x+1$]
\label{ex:x4quartic}
Let $p=x^4-x+1$: irreducible, no real roots, non-reciprocal --- hence two
conjugate-pair shells at radii $r\neq1/r$, and \emph{no} modulus of $p$ is
uniquely attained, so Theorem~\ref{thm:pinning} is silent. Three exact
routes settle the full type (ledger entry~X). \emph{Structure:} the
Kronecker square factors as
$\operatorname{charpoly}(C_p\otimes C_p)=S_6(x)^2\,(x^4+2x^2-x+1)$, with
$S_6$ the certified Salem sextic of Theorem~\ref{thm:inertmore} and the
second factor the $\psi^2$-image, irreducible since
$\alpha=(\alpha^2)^2+1$. So the pair products are
$\{\alpha\overline\alpha,\,\beta\overline\beta\}
=\{\tau_{S_6},\,1/\tau_{S_6}\}$ and the four cross products
$\alpha\overline\beta,\dots$ are $S_6$'s unit-circle conjugates; a
cross-shell pair of $p$ is coherent iff its unimodular product is a root of
unity --- impossible for a circle conjugate of a Salem polynomial.
Cross-shell incoherence is therefore forced \emph{through $S_6$'s own
rigidity}. \emph{Shell scan:} $\Rat_p$ has contacts $\{\Phi_1^{\,4}\}$,
complete --- no within-shell torsion, no rational angles. \emph{The
criterion verbatim:} $\Rat_{\Rat_p}$ has degree $256$; its complete contact
scan (bound $131072$) returns exactly $\{\Phi_1^{\,28}\}$ --- the
multiplicity $16+4+8=28$ predicted from the coincidence-free structure
\emph{before} the scan ran. Verdict: $p$ is fully relationally inert \F{}
--- the first complete type where neither pinning nor the shell scan alone
suffices, decided by rigidity imported across $\otimes$, the converse
motion of Example~\ref{ex:tensorsq}. Two by-products.
$\Mah(x^4-x+1)=\tau_{S_6}$: the smallest degree-$6$ Salem number \emph{is}
this quartic's Mahler measure --- a value in $(\theta_0,\varphi)$ attained
at irrational angles, consistent with \textup{(EG)}; which numbers arise as
Mahler measures is Boyd's program~\cite{Boyd}, and the identity may well be
recorded in the small-measure tables. And $\mathrm{Gal}(p)=S_4$: a fully
generic quartic with a completely rigid relational type.
\end{example}
...
\begin{remark}[Degeneracy, the Dubickas program, and attribution]
\label{rem:degen}
...
(see~\cite{EPSW}). A targeted registry search conducted for this revision
(Crossref and the arXiv API, 2~July~2026; terms: \emph{quotients of
conjugate algebraic numbers root of unity}; \emph{roots of unity as
quotients of two roots of a polynomial}; \emph{multiplicative relations
conjugates Salem}; \emph{degenerate recurrence dominant root}; MathSciNet
and zbMATH were not available to us) shows the torsion-quotient question to
be an established program of Dubickas: quantitative bounds on the least $T$
such that no quotient of two distinct $T$-th powers of roots of an
arbitrary $f\in K[x]$ is a root of unity~\cite{Dub2012} --- a framing that
encompasses pairs across irreducible factors and is motivated there by the
(non)degeneracy of recurrences --- together with~\cite{Dub2017, Dub2021} on
quotients of two conjugates. Following review, the full text
of~\cite{Dub2017} (open access) was read: its theorem is the quantitative
totient bound $\phi(n_1\cdots n_r)\le d$ on the coprime part of the
degenerate set, sharpening the prime count of Dobrowolski's
Lemma~3~\cite{Dobrowolski} (a textbook treatment is Masser~\cite{Masser});
the degenerate-recurrence equivalence is stated there explicitly, and its
proof shares the Galois-transport step with ours --- torsion quotients
moved by chosen automorphisms --- which we take as evidence the mechanism
is standard in that program. The pinned-modulus sufficient condition and
the mixed two-polynomial form do not appear in~\cite{Dub2017};
\cite{Dub2012} and~\cite{Dub2021} remain consulted at title and abstract
level only (paywalled), as do the works cited therein --- to the extent the
criterion appears in any of them, priority is theirs and
Theorem~\ref{thm:pinning} should be read as a self-contained account. What this note
claims as its own is the packaging: the pinned-modulus formulation as used
here with its mixed two-polynomial form, the coherence-type invariant and
its shell-complete decision procedure, the census infrastructure, and the
Salem/Pisot corollaries in relational form --- corroborated on forty-six
instances before the general argument was written down. The multiplicative
rigidity of Corollary~\ref{cor:allinert} also sits in instructive contrast
with the \emph{additive} theory: Dubickas--Jankauskas~\cite{DJ2020} show
nontrivial linear relations among Salem conjugates do exist (from
degree~$8$, descending to relations among conjugates of $\tau+1/\tau$ ---
the same trace substitution that certifies our instances), whereas torsion
multiplicative relations never do.
\end{remark}
...
of ledger entries A--I, together with rows P, Q, U, V, and the round-eight
rows of entry~X, were
recomputed independently in PARI/GP~2.15.4 via
\texttt{polresultant}, integer factorization, and \texttt{poliscyclo}; all
twenty-six items agree with the \texttt{sympy} computations ---
twenty-four contact signatures plus the Kronecker-square factorization and
divisibility checks of entry~X (ledger
...
operator is not. The polynomial inputs are generated programmatically from
coefficient vectors, confining operator-dependent transcription to the
enumeration logic --- itself validated by the Burnside count of
Theorem~\ref{thm:census} matching the enumeration ($378$ twist-classes).
Independent replication by external researchers is welcomed:
...
\texttt{supplement\_round7.py}, \texttt{supplement\_round8.py},
\texttt{cross\_check.gp}, \texttt{cross\_round8.gp},
\texttt{cross\_round6.gp}, \texttt{SHA256SUMS}); the project repository is DOI-archived at
\url{10.5281/zenodo.21121863}; the scripts above now ship in that deposit
(added in v1.0.1, version DOI \url{10.5281/zenodo.21122410}, in the
\texttt{relational-charge-verification/} package alongside this
manuscript), enabling verification by a distinct operator. Concretely (expected
...
\item \texttt{python3 supplement\_round7.py} --- entry W (plastic number;
census rejection tally; symbolic $\nu$-criterion);
\item \texttt{python3 census\_deg12.py} --- the census of
Theorem~\ref{thm:census} (about two minutes);
\item \texttt{python3 supplement\_round8.py} --- entry X (the quartic
$x^4-x+1$: Kronecker-square factorization and the complete $\nu$-scan);
\item \texttt{gp -q cross\_check.gp}, \texttt{gp -q cross\_round6.gp}, and
...
\texttt{pisot-residue-verification.zip} (SHA-256 \texttt{f803ce52dbdfd070}\allowbreak\texttt{390df72749c3e778}\allowbreak\texttt{29783e732a5f231e}\allowbreak\texttt{2d60e31be9160404})
\end{itemize}
...
pinned/twisted-shell dichotomy (cf.~\cite{EPSW}; the quantitative side of
exactly this question is developed in~\cite{Dub2012}), and, beyond torsion,
...
$(\varphi,2)$, maximally empty. Beyond classification, the types earn their
keep three ways: rigidity transports across $\otimes$
(Example~\ref{ex:x4quartic} decides a generic-Galois quartic through a
Salem factor of its tensor square); the contact machinery doubles as an
exact nondegeneracy certifier for integer linear recurrences
(Remark~\ref{rem:degen}); and type computations surface measure identities
such as $\Mah(x^4-x+1)=\tau_{S_6}$.
...
M & cross-engine replication & entries A--I, P, Q, U, V, and the round-8
rows of entry~X recomputed in PARI/GP~2.15.4 (\texttt{polresultant} $+$
\texttt{poliscyclo}); all twenty-six items identical to the \texttt{sympy}
results (twenty-four signatures, two factorization checks). Live replication
(run~3, post-archive): the signature ledger recomputed end-to-end in
PARI/GP~2.15.4 under engine-native census selection, each scan executed on
two independent in-engine paths (trial division vs.\ factorization);
$237/237$ signatures agree across $474$ scan executions, $0$ disagreements
(\C).\\[2pt]
...
(Remark~\ref{rem:censusscope}).\\[2pt]
W & plastic number; tally & $x^3-x-1$: irreducible, one real root $>1$
(Sturm), pair modulus $\theta_0^{-1/2}$; contacts $\{\Phi_1^{\,3}\}$ ---
inert (Example~\ref{ex:pisotnu}). Census rejection tally re-derived:
$39/257/45/0/37$, sum $378$. Single-engine
(\texttt{supplement\_round7.py}).\\[2pt]
X & $x^4-x+1$ ($\nu$ executed) & irreducible, $0$ real roots (Sturm),
non-reciprocal; $\operatorname{charpoly}(C\otimes C)
=S_6^{\,2}\cdot(x^4+2x^2-x+1)$, both engines; $\Rat_p$ contacts
$\{\Phi_1^{\,4}\}$; $\Rat_{\Rat_p}$ (deg $256$, bound $131072$) contacts
$\{\Phi_1^{\,28}\}$, matching the structural prediction;
$\mathrm{Gal}=S_4$; $\Mah=\tau_{S_6}$ (Example~\ref{ex:x4quartic}).\\
\hline
\end{tabular}
\end{center}

\medskip
\noindent\textbf{Engine-harness rules (PARI/GP).}
Two failure modes of the \texttt{gp} interpreter were isolated during the
live replications; both are properties of the harness, never of the
mathematics, and both are machine-demonstrated (\F). They are recorded here
beside the engine pins (\texttt{sympy}~1.14.0 / Python~3.12.3 /
PARI/GP~2.15.4) as binding rules for every \texttt{gp}-side artifact of this
deposit and its successors.
\begin{itemize}
\item[\textbf{H1.}] \texttt{gp} terminates top-level statements at newline: a
function body split across physical lines without braces silently truncates.
\emph{Rule:} multi-line \texttt{gp} definitions must be brace-wrapped;
single-line otherwise.
\item[\textbf{H2.}] \texttt{gp} exits $0$ after load errors, emitting
\texttt{***} diagnostics to stderr only (``skipping file'').
\emph{Rule:} harnesses must treat any non-benign \texttt{***} stderr line as
fatal (the canonical benign line is the \texttt{parisize} stack-resize
notification).
\end{itemize}
...
\bibitem{pisotnote} J.~Turner, \emph{The Pisot Cross-Shell Residue: A
...
\texttt{f803ce52dbdfd070}\allowbreak\texttt{390df72749c3e778}\allowbreak\texttt{29783e732a5f231e}\allowbreak\texttt{2d60e31be9160404}) and its N2--N4 successors.
...
\bibitem{Dub2012} A.~Dubickas, \emph{Roots of unity as quotients of two
roots of a polynomial}, J.~Aust.\ Math.\ Soc.\ 92 (2012), 137--144.

\bibitem{Dub2017} A.~Dubickas, \emph{Roots of unity as quotients of two
conjugate algebraic numbers}, Glas.\ Mat.\ 52 (2017), 235--240.

\bibitem{Dub2021} A.~Dubickas, \emph{Cyclotomic quotients of two
conjugates of an algebraic number}, Sib.\ Math.\ J.\ 62 (2021), 409--412.

\bibitem{DJ2020} A.~Dubickas and J.~Jankauskas, \emph{Linear relations
with conjugates of a Salem number}, J.~Th\'eor.\ Nombres Bordeaux 32
(2020), 179--191.

\bibitem{Masser} D.~Masser, \emph{Auxiliary Polynomials in Number Theory},
...
machine-checked at DOI \url{10.1090/surv/104}. References \cite{Dub2012},
\cite{Dub2017}, \cite{Dub2021}, and \cite{DJ2020}, added at this revision
following the novelty search of Remark~\ref{rem:degen}, were machine-checked
at DOIs \url{10.1017/s1446788712000183}, \url{10.3336/gm.52.2.03} (this
record's registry author field is partially malformed; the resolvable
author is retained), \url{10.1134/s0037446621030034}, and
\url{10.5802/jtnb.1116}; abstracts, where present in the registry, were
consulted. The full text of \cite{Dub2017} was subsequently read from the
journal's open-access copy (Remark~\ref{rem:degen}); \cite{Masser}, located
through it, was machine-checked at the book DOI
\url{10.1017/cbo9781107448018}.
