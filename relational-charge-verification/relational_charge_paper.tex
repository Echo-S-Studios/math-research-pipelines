...
$t(\alpha)=\frac{1}{2\pi}\Arg\alpha$, defined when rational --- and determine exactly how much
...
are reducible (none fail only trace-polynomial irreducibility); the tally
sums to $378$ (ledger entry~W).
\end{remark}
...
quotients of two conjugates. Only titles, registry metadata, and abstracts
(where the registry carries them) were consulted, per the project's
provenance discipline; to the extent the pinned-modulus criterion of
Theorem~\ref{thm:pinning} appears in those works, priority is theirs and
the theorem should be read as a self-contained account. What this note
...
\texttt{supplement\_round7.py}, \texttt{cross\_check.gp},
